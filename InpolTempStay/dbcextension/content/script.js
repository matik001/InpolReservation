...
function doActionsOnSuccess(msg) {
    ...
    Config.getAll().then(config => {
        ...
        if (callback) {
            ...
            textarea.id = 'dbc-callback-trigger';
...
function executeAutoSubmitRule(rules) {
    for (let i = 0; i < rules.length; i++) {
        ...
        if (regExp.test(location.href)) {
            let textarea = document.createElement('textarea');
            textarea.id = 'dbc-autosubmit-code';
    ...
    let button = $(`
        <div class="captcha-solver captcha-solver_${config.buttonPosition}" data-state="ready" data-captcha-type="${captchaType}" data-widget-id="${widgetId}">
            <div class="captcha-solver-image">
                <img src="${chrome.runtime.getURL("assets/images/icon_32-white.png")}">
            </div>
            <div class="captcha-solver-info">${chrome.i18n.getMessage("solveWithDeathByCaptcha")}</div>
        </div>
    `);
...
function showToast(message) {
    if (!$('body > .dbc-toast-container').length) {
        $('body').append(`<div class="dbc-toast-container"></div>`);
    }

    let toastEl = $(`
        <div class="dbc-toast">
            <img src="${chrome.runtime.getURL("assets/images/logo.png")}" class="dbc-toast-logo">
            <button type="button" class="close">&times;</button>
            <div class="dbc-toast-message">${message}</div>
        </div>
    `).appendTo('.dbc-toast-container');
